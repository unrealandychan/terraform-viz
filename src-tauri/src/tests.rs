...
mod tests {
    ...
    /// Golden test: compare Rust parser output against TS parser expected output.
    ///
    /// The golden file (tests/golden-ts-expected.json) was generated by running
    /// the TypeScript buildGraphModel on every example file.
    /// We compare nodeCount, sorted nodeIds, edgeCount, and sorted edges.
    ///
    /// Any mismatch means the Rust parser diverges from the TS parser.
    #[test]
    fn rust_parser_matches_ts_parser_golden() {
        let examples_dir = concat!(env!("CARGO_MANIFEST_DIR"), "/../examples");
        let golden_path = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/golden-ts-expected.json");

        let golden_raw = std::fs::read_to_string(golden_path)
            .expect("golden-ts-expected.json not found — run scripts/run-ts-parser.ts to regenerate");
        let golden: serde_json::Value = serde_json::from_str(&golden_raw)
            .expect("invalid golden JSON");
        let golden_entries = golden.as_array().expect("golden must be an array");

        let mut failures: Vec<String> = Vec::new();

        for entry in golden_entries {
            let file = entry["file"].as_str().unwrap();
            let expected_node_count = entry["nodeCount"].as_u64().unwrap() as usize;
            let expected_node_ids: Vec<String> = entry["nodeIds"]
                .as_array().unwrap()
                .iter().map(|v| v.as_str().unwrap().to_string()).collect();
            let expected_edge_count = entry["edgeCount"].as_u64().unwrap() as usize;
            let expected_edges: Vec<String> = entry["edges"]
                .as_array().unwrap()
                .iter().map(|v| v.as_str().unwrap().to_string()).collect();

            let plan_path = std::path::Path::new(examples_dir).join(file);
            let raw = std::fs::read_to_string(&plan_path)
                .unwrap_or_else(|e| panic!("failed to read {}: {}", file, e));
            let model = parse_plan(&raw)
                .unwrap_or_else(|e| panic!("parse_plan failed for {}: {}", file, e));

            // Build actual sorted values
            let mut actual_node_ids: Vec<String> = model.nodes.iter().map(|n| n.id.clone()).collect();
            actual_node_ids.sort();
            let mut actual_edges: Vec<String> = model.edges
                .iter().map(|e| format!("{} -> {}", e.source, e.target)).collect();
            actual_edges.sort();

            // Compare node count
            if model.nodes.len() != expected_node_count {
                // Find missing / extra nodes
                let missing: Vec<&String> = expected_node_ids.iter()
                    .filter(|id| !actual_node_ids.contains(id)).collect();
                let extra: Vec<&String> = actual_node_ids.iter()
                    .filter(|id| !expected_node_ids.contains(id)).collect();
                failures.push(format!(
                    "❌ {}: nodes expected={} got={}\n   missing: {:?}\n   extra:   {:?}",
                    file, expected_node_count, model.nodes.len(), missing, extra
                ));
            }

            // Compare edges
            if actual_edges != expected_edges {
                let missing: Vec<&String> = expected_edges.iter()
                    .filter(|e| !actual_edges.contains(e)).collect();
                let extra: Vec<&String> = actual_edges.iter()
                    .filter(|e| !expected_edges.contains(e)).collect();
                failures.push(format!(
                    "❌ {}: edges expected={} got={}\n   missing: {:?}\n   extra:   {:?}",
                    file, expected_edge_count, actual_edges.len(), missing, extra
                ));
            }

            if model.nodes.len() == expected_node_count && actual_edges == expected_edges {
                println!("✅ {}: nodes={} edges={}", file, model.nodes.len(), actual_edges.len());
            }
        }

        if !failures.is_empty() {
            panic!(
                "\n\n{} golden comparison failure(s):\n\n{}\n",
                failures.len(),
                failures.join("\n\n")
            );
        }
